...
function paragraph(
  ...
  const properties = [
    ...
    options.before != null || options.after != null
      ...
      : '',
    options.left != null || options.hanging != null
      ? `<w:ind w:left="${options.left ?? 0}"${options.hanging != null ? ` w:hanging="${options.hanging}"` : ''}/>`
      : '',
...
function section(label: string) {
  return paragraph(label.toUpperCase(), { style: 'SectionHeading', keepNext: true, before: 35, after: 10 });
}

function subBullet(text: string) {
  return paragraph([run('◦ '), run(text)], { style: 'SubBullet', after: 4 });
}
...
function pairedLine(left: string | string[], right: string, options: { style?: string; keepNext?: boolean; italicLeft?: boolean } = {}) {
  ...
  ], { style: options.style, keepNext: options.keepNext, after: 0, rightTab: 10900 });
}
...
function skillsParagraphs(profile: CandidateProfile, pack: ApplicationPack) {
  ...
  for (const group of profile.skillGroups ?? []) {
    ...
    paragraphs.push(paragraph([
      run('• '), run(`${group.label}: `, { bold: true }), run(skills.join(', ')),
    ], { after: 3 }));
  }
  const extras = pack.skills.filter((skill) => !rendered.has(normalizeText(skill)));
  if (extras.length) paragraphs.push(paragraph([
    run('• '), run(paragraphs.length ? 'Additional: ' : 'Skills: ', { bold: true }), run(extras.join(', ')),
  ], { after: 3 }));
  return paragraphs;
}
...
function documentBody(profile: CandidateProfile, pack: ApplicationPack) {
  const body: string[] = [
    paragraph(profile.name, { style: 'Name', align: 'center', after: 18 }),
  ];
  if (pack.resumeHeadline.trim()) body.push(paragraph(pack.resumeHeadline, { style: 'Headline', align: 'center', after: 10 }));
  const contact = resumeTemplateContactItems(profile);
  if (contact.length) body.push(paragraph(contact.join('   '), { style: 'Contact', align: 'center', after: 20 }));

  body.push(section('Professional Summary'), paragraph(pack.resumeSummary, { after: 12 }));
  ...
  if (pack.certifications?.length) {
    ...
    pack.certifications.forEach((certification) => body.push(paragraph([run('• '), run(certification)], { after: 2 })));
  }
  // Publications are intentionally not rendered under any circumstance.
  return body.join('');
}
...
<w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:docDefaults><w:rPrDefault><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:eastAsia="Times New Roman" w:cs="Times New Roman"/><w:sz w:val="17"/><w:szCs w:val="17"/><w:lang w:val="en-CA"/></w:rPr></w:rPrDefault><w:pPrDefault><w:pPr><w:spacing w:after="0" w:line="190" w:lineRule="auto"/></w:pPr></w:pPrDefault></w:docDefaults>
  <w:style w:type="paragraph" w:default="1" w:styleId="Normal"><w:name w:val="Normal"/><w:qFormat/></w:style>
  <w:style w:type="paragraph" w:styleId="Name"><w:name w:val="Name"/><w:basedOn w:val="Normal"/><w:qFormat/><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman"/><w:b/><w:sz w:val="47"/><w:szCs w:val="47"/></w:rPr></w:style>
  <w:style w:type="paragraph" w:styleId="Headline"><w:name w:val="Headline"/><w:basedOn w:val="Normal"/><w:rPr><w:b/><w:sz w:val="18"/><w:szCs w:val="18"/></w:rPr></w:style>
  ...
  <w:style w:type="paragraph" w:styleId="RoleLine"><w:name w:val="Role Line"/><w:basedOn w:val="Normal"/><w:pPr><w:ind w:left="220"/></w:pPr><w:rPr><w:sz w:val="17"/><w:szCs w:val="17"/></w:rPr></w:style>
  <w:style w:type="paragraph" w:styleId="SubBullet"><w:name w:val="Sub Bullet"/><w:basedOn w:val="Normal"/><w:pPr><w:ind w:left="500" w:hanging="180"/><w:spacing w:after="4"/></w:pPr><w:rPr><w:sz w:val="17"/><w:szCs w:val="17"/></w:rPr></w:style>
</w:styles>`;
...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${documentBody(profile, pack)}<w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="360" w:right="400" w:bottom="300" w:left="400" w:header="0" w:footer="0" w:gutter="0"/></w:sectPr></w:body></w:document>`);
