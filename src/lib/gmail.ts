...
export function gmailConfigured() {
  return Boolean(process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET && process.env.GOOGLE_REFRESH_TOKEN && process.env.GMAIL_DIGEST_TO);
}
...
export async function createOutreachDraft(subject: string, text: string, to?: string) {
  ...
  const headers = [to ? `To: ${to}` : null, `Subject: ${subject}`, 'MIME-Version: 1.0', 'Content-Type: text/plain; charset="UTF-8"', '', text].filter((value): value is string => value !== null);
