...
export function JobListClient({ jobs }: { jobs: JobWithMatch[] }) {
  ...
  const visible=useMemo(()=>jobs.filter(j=>{
    ...
    return match&&category&&sourceHit&&stageHit&&typeHit&&validityHit;
  }).sort((a,b)=>validityRank(b.validityStatus)-validityRank(a.validityStatus)
    || (b.healthScore ?? 50)-(a.healthScore ?? 50)
    || (b.match?.overall ?? -1)-(a.match?.overall ?? -1)),[jobs,q,filter,source,careerStage,jobType,validity]);
  const totalPages=Math.max(1,Math.ceil(visible.length/PAGE_SIZE));
  const paged=useMemo(()=>visible.slice((page-1)*PAGE_SIZE,page*PAGE_SIZE),[visible,page]);

  useEffect(()=>setPage(1),[q,filter,source,careerStage,jobType,validity]);
  ...
  return <>
    ...
    {!visible.length?<div className="notice">No jobs match the selected filters. Try another job type, stage, posting state, source, or search term.</div>:null}
    <div className="table-wrap"><table><thead><tr><th>Opportunity</th><th>Type</th><th>Location</th><th>Posting health</th><th>Match</th><th>Decision</th><th>Status</th><th></th></tr></thead><tbody>{paged.map(job=>{const labels=jobTypeLabels(job);return <tr key={job.id}><td><div className="job-title">{job.title}</div><div className="job-company">{job.company} · {job.source}</div></td><td>{labels.length?<div className="tag-list">{labels.map(label=><span className="tag" key={label}>{label}</span>)}</div>:<span className="muted">—</span>}</td><td>{job.location||'—'}</td><td><span className="pill">{validityLabel(job.validityStatus)}</span><div className="small muted">{job.healthScore ?? 50}/100</div></td><td><span className="score">{job.match?.overall ?? '—'}</span>{job.match?<span className="muted">/100</span>:null}</td><td>{job.match?<StatusPill value={job.match.recommendation}/>:<span className="pill">unanalyzed</span>}</td><td><span className="small">{job.application?.status||'discovered'}</span></td><td><a className="btn ghost" href={`/jobs/${job.id}`}>Review →</a></td></tr>})}</tbody></table></div>
