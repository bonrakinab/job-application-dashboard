...
export function JobListClient({ jobs, fixedJobType }: { jobs: JobWithMatch[]; fixedJobType?: Exclude<JobTypeFilter, 'all'> }) {
  ...
  return <>
    ...
    <div className="table-wrap"><table><thead><tr><th>Job</th><th>Application</th><th>Type</th><th>Location</th><th>Posting</th><th>Match</th><th>Decision</th><th></th></tr></thead><tbody>{paged.map(job=>{const labels=jobTypeLabels(job);return <tr key={job.id}><td><div className="job-title">{job.title}</div><div className="job-company">{job.company}</div>{job.yc ? <div className="tag-list"><span className="tag">YC {job.yc.batch || 'startup'}</span>{job.match?.startupFit != null ? <span className="tag">Startup fit {job.match.startupFit}</span> : null}</div> : null}</td><td><span className="tag"><b>{applicationLabel(job.application)}</b></span></td><td>{labels.length?<div className="tag-list">{labels.slice(0,2).map(label=><span className="tag" key={label}>{label}</span>)}</div>:<span className="muted">—</span>}</td><td>{job.location||'—'}</td><td><span className="pill">{validityLabel(job.validityStatus)}</span></td><td><span className="score">{job.match?.overall ?? '—'}</span>{job.match?<span className="muted">/100</span>:null}</td><td>{job.match?<StatusPill value={job.match.recommendation}/>:<span className="pill">unanalyzed</span>}</td><td><a className="btn ghost" href={`/jobs/${job.id}`}>View</a></td></tr>})}</tbody></table></div>
