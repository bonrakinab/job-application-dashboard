...
export function JobListClient({ jobs }: { jobs: JobWithMatch[] }) {
  ...
  return <>
    <div className="searchbar job-filters">
      <input className="input" placeholder="Search title, company, location…" value={q} onChange={e=>setQ(e.target.value)}/>
      <select className="select" value={filter} onChange={e=>setFilter(e.target.value)}><option value="all">All scores</option><option value="exceptional">Exceptional</option><option value="strong">Strong</option><option value="reasonable">Reasonable</option><option value="stretch">Stretch</option><option value="skip">Skip</option></select>
      <select className="select" value={careerStage} onChange={e=>setCareerStage(e.target.value)}><option value="all">All stages</option><option value="internship">Internships</option><option value="new-grad">New grad</option><option value="entry-level">Entry level</option><option value="experienced">Experienced</option></select>
      <select className="select" aria-label="Job type" value={jobType} onChange={e=>setJobType(e.target.value as JobTypeFilter)}><option value="all">All job types</option><option value="full-time">Full-time</option><option value="part-time">Part-time</option><option value="contract">Contract / contractual</option><option value="remote">Remote</option><option value="hybrid">Hybrid</option><option value="on-site">On-site / non-remote</option></select>
      <select className="select" aria-label="Application state" value={applicationState} onChange={e=>setApplicationState(e.target.value as ApplicationFilter)}><option value="all">Applied + not applied</option><option value="applied">Applied only</option><option value="not-applied">Not applied only</option></select>
      <select className="select" value={validity} onChange={e=>setValidity(e.target.value)}><option value="viable">Viable + unverified</option><option value="verified">Verified / likely active</option><option value="unknown">Unverified only</option><option value="closed">Closed / likely closed</option><option value="all">All posting states</option></select>
      <select className="select" value={source} onChange={e=>setSource(e.target.value)}><option value="all">All sources</option>{sources.map(value=><option value={value} key={value}>{value}</option>)}</select>
    </div>
    ...
    <div className="table-wrap"><table><thead><tr><th>Opportunity</th><th>Application</th><th>Type</th><th>Location</th><th>Posting health</th><th>Match</th><th>Decision</th><th></th></tr></thead><tbody>{paged.map(job=>{const labels=jobTypeLabels(job);return <tr key={job.id}><td><div className="job-title">{job.title}</div><div className="job-company">{job.company} · {job.source}</div></td><td><span className="tag"><b>{applicationLabel(job.application)}</b></span></td><td>{labels.length?<div className="tag-list">{labels.map(label=><span className="tag" key={label}>{label}</span>)}</div>:<span className="muted">—</span>}</td><td>{job.location||'—'}</td><td><span className="pill">{validityLabel(job.validityStatus)}</span><div className="small muted">{job.healthScore ?? 50}/100</div></td><td><span className="score">{job.match?.overall ?? '—'}</span>{job.match?<span className="muted">/100</span>:null}</td><td>{job.match?<StatusPill value={job.match.recommendation}/>:<span className="pill">unanalyzed</span>}</td><td><a className="btn ghost" href={`/jobs/${job.id}`}>Review →</a></td></tr>})}</tbody></table></div>
