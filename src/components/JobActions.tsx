...
import { RESUME_TEMPLATE_VERSION } from '@/lib/resume-template';
...
export function JobActions({
  ...
  async function verifyAndOpen() {
    ...
    try {
      ...
      if (verifiedStatus === 'closed' || verifiedStatus === 'likely_closed') { popup?.close(); setMsg(json.closureReason || 'This posting appears closed, so the application link was not opened.'); return; }
      if (verifiedStatus === 'unknown') { popup?.close(); setMsg('The posting could not be confirmed as active. It was not opened automatically; use the original source link if you want to inspect it manually.'); return; }
  ...
  return <div className="card application-actions">
    ...
    <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 6 }}>
      <span className="support-pill support-supported">Resume engine {engineLabel}</span>
      <span className={`small ${usablePack ? 'text-success' : hasPack ? 'text-warning' : 'muted'}`}>
        {usablePack ? '✓ Latest version active for this job' : hasPack ? 'New version available — regenerate this job' : 'Latest version will be used when generated'}
      </span>
    </div>
