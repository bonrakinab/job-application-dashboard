...
async function fetchHimalayas(): Promise<Job[]> {
  ...
  const deduped = [...new Map(rows.filter((item: any) => item.guid).map((item: any) => [item.guid, item])).values()] as any[];
  return Promise.all(deduped.filter((item) => item.guid && item.title && item.companyName && item.applicationLink).map(async (item) => ({
    id: await stableJobId('himalayas', 'public-api', String(item.guid)), external_id: String(item.guid), source: 'himalayas', source_key: 'public-api', url: item.applicationLink, apply_url: item.applicationLink, title: item.title, company: item.companyName, location: Array.isArray(item.locationRestrictions) && item.locationRestrictions.length ? item.locationRestrictions.join(', ') : 'Worldwide', description: stripHtml(item.description || item.excerpt || ''), posted_at: item.pubDate, salary_min: item.minSalary ?? undefined, salary_max: item.maxSalary ?? undefined, currency: item.currency ?? undefined, salary_text: item.minSalary || item.maxSalary ? `${item.currency ?? ''} ${item.minSalary ?? ''}${item.maxSalary ? `–${item.maxSalary}` : ''} ${item.salaryPeriod ?? ''}`.trim() : undefined, employment_type: item.employmentType, remote: true, workplace_type: 'Remote', department: Array.isArray(item.category) ? item.category.join(', ') : undefined, raw: { sourceAttribution: 'Himalayas', companySlug: item.companySlug },
  })));
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const errors: string[] = [];
    const allJobs: Job[] = [];
    const settled = await Promise.allSettled([fetchRemoteOk(), fetchHimalayas()]);
    const sourceNames = ['Remote OK', 'Himalayas'];
    ...
    await db('activity_log', { method: 'POST', headers: { Prefer: 'return=minimal' }, body: JSON.stringify([{ event: 'expanded_remote.discovery.completed', payload: { fetched: deduped.length, relevant: relevant.length, sources: sourceNames.length, errors }, created_at: now }]) });
